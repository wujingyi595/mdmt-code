...
class KeypointEncoder(nn.Module):
    ...
    def __init__(self, keypoint_dim: int, feature_dim: int, layers: List[int], dropout: bool = False,
                 p: float = 0.1) -> None:
        super().__init__()
    ...
    def __init__(self, config, dropout=False, p=0.1, use_kenc=True, use_cross=True):
        ...
        if use_kenc:
            self.kenc = KeypointEncoder(
                self.config['keypoint_dim'], self.config['descriptor_dim'], self.config['keypoint_encoder'],
                dropout=dropout)
