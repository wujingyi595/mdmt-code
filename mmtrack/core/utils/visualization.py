...
def crop_show_tracks(img,
    ...
    labels = labels[inds]
    ids = ids[inds]
    dir = r'./fast_reid_master/datasets/reid/'
    if file_type == 'delete' :
        query_path = osp.join(dir,r'test_imgs')
    ...
    if file_type == 'query' :
        ...
        #     shutil.rmtree(query_path) 
        # os.mkdir(query_path)
        txt_path  = osp.join(dir,'query_1_small.txt')
        if os.path.exists(txt_path):
            os.remove(txt_path)
        f=open(txt_path,'w')
    else:
        gallery_path = osp.join(dir,r'test_imgs')
        # shutil.rmtree(gallery_path) 
        # os.mkdir(gallery_path)
        txt_path  = osp.join(dir,'gallery_2_small.txt')
        if os.path.exists(txt_path):
            os.remove(txt_path)
        f=open(txt_path,'w')      
    for i, (bbox, label, id) in enumerate(zip(bboxes, labels, ids)):
        ...
        cut_color_img = img[y1:y2, x1:x2]
        # '%04d'%(int(id))
        # save_name = '{}.jpg'.format('%04d'%(int(id)))
        
        if file_type == 'query' :
            save_name = 'qu-1_{}.jpg'.format('%06d'%(int(id)))

            file_path = osp.join(query_path,str('%04d'%(int(id))))
            if not os.path.isdir(file_path):
                os.mkdir(file_path)
            cv2.imwrite(file_path+'/'+save_name,cut_color_img)
            new_context = r'{}/{} {}'.format(str('%04d'%(int(id))), save_name,i)
            f.write(new_context)
            f.write('\r\n')
        else :
            save_name = 'ga-2_{}.jpg'.format('%06d'%(int(id)))
            file_path = osp.join(gallery_path,str('%04d'%(int(id))))
            if not os.path.isdir(file_path):
                os.mkdir(file_path)
            cv2.imwrite(file_path+'/'+save_name,cut_color_img)
            new_context = r'{}/{} {}'.format(str('%04d'%(int(id))), save_name,i)           
