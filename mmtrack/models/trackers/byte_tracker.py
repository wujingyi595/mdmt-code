...
#             # 0. init
#             if 'bil' in kwargs:
#                 # print("frame_0 getten from labels")
#                 bboxess = kwargs['bil']['bboxes1']
#                 labelss = kwargs['bil']['labels1']
#                 idss = kwargs['bil']['ids1']
#                 # frame_id = i - 1的问题：因为原先的代码中update在inference之后更新，在inference计算前相当于使用的缓存区中最后一帧是frame-1的，而修改代码之后是先update，这样会更改缓存区中的最后一帧为frame，这样在后续的计算会出错
#                 self.update(ids=idss, bboxes=bboxess, labels=labelss, frame_ids=frame_id-1)
#                 # print(bboxes, labels, ids)
#             ids = torch.full((bboxes.size(0), ),
#                              -1,
#                              dtype=labels.dtype,
#                              device=labels.device)
...
#                 case_1 = first_match_track_inds[i] == -1
#                 # tracklet is not lost in the previous frame
#                 case_2 = self.tracks[id].frame_ids[-1] == frame_id - 1   # 总是true?
#                 if case_1 and case_2:
#                     first_unmatch_track_ids.append(id)
...
#             # for cc in range(len(self.tracks)):
#             # print(max(self.tracks.keys()))
#             ids[new_track_inds] = torch.arange(
#                 max(self.tracks.keys())+1,
#                 max(self.tracks.keys())+1 + new_track_inds.sum()).to(labels)
#             self.num_tracks += new_track_inds.sum()

#         if frame_id==0 and ('bil' in kwargs):
#             print("frame_0 getten from labels")
#             bboxes = kwargs['bil']['bboxes1']
#             labels = kwargs['bil']['labels1']
#             ids = kwargs['bil']['ids1']
#             self.update(ids=ids, bboxes=bboxes, labels=labels, frame_ids=0)
#             # print(bboxes, labels, ids)

#         # self.update(ids=ids, bboxes=bboxes, labels=labels, frame_ids=frame_id)  # 在此先不更新confirmedID的信息，因为现在更新了就会被写进去而且在前端双机融合的时候只能添加不能删除此处update的confirmedID,导致tracker的时候跟无用的ID追踪上
#         return bboxes, labels, ids
...
#             ids[new_track_inds] = torch.arange(
#                 max_id+1,
#                 max_id+1 + new_track_inds.sum()).to(labels)
#             max_id = max_id + 1 + new_track_inds.sum()
#             self.num_tracks += new_track_inds.sum()
...
#         # self.update(ids=ids, bboxes=bboxes, labels=labels, frame_ids=frame_id)  # 在此先不更新confirmedID的信息，因为现在更新了就会被写进去而且在前端双机融合的时候只能添加不能删除此处update的confirmedID,导致tracker的时候跟无用的ID追踪上
#         return bboxes, labels, ids, max_id
