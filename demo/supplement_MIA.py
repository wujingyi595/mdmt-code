...
import json
import time
from utils.matching_pure import matching, calculate_cent_corner_pst, draw_matchingpoints
...
def main():
    ...
    parser.add_argument('--method', default='NMS-one_carafe_bytetrack_full_mdmt', help='the output directory name used in result_dir')
    ...
    parser.add_argument('--checkpoint', help='checkpoint file, can be initialized in config files')  # , default="../workdirs/autoassign_epoch_60.pth"
    parser.add_argument(
        '--score-thr',
        type=float,
        default=0.0,
    ...
    for dirrr in sorted(os.listdir(args.input)):
        ...
        for i, img in enumerate(imgs):
            ...
            result, max_id = inference_mot(model, img, frame_id=i, bboxes1=bboxes1, ids1=ids1, labels1=labels1,
                                           max_id=max_id)
            # print(result['det_bboxes'])
            # print(result['det_bboxes'][0])
            ...
            for bbox, id in zip(bboxes1.numpy(), ids1.numpy()):
                ...
                # 在图像上绘制标签
                cv2.putText(image1, str(id.item()), (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            for bbox, id in zip(bboxes2.numpy(), ids2.numpy()):
                x1, y1, x2, y2 = bbox
                # 在图像上绘制边界框
                cv2.rectangle(image2, (x1, y1), (x2, y2), (0, 255, 0), 2)
                # 在图像上绘制标签
                cv2.putText(image2, str(id.item()), (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            os.makedirs('./imagetrackbox/'+str(dirrr), exist_ok=True)
            cv2.imwrite("./imagetrackbox/{}/boxesA-{}.jpg".format(dirrr,i), image1)
