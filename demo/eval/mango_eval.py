...
def read_gt(gt_path, camera):
    ...
    for i, line in enumerate(lines):
        ...
        targetObject = GTTarget(i, int(attributes[0])-1, camera, attributes[1], attributes[2], attributes[3], int(float(attributes[2]))+int(float(attributes[4])), int(float(attributes[3]))+int(float(attributes[5])))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sequences_result', default="./json_resultfiles2/supplement_supplement/NMS-one_carafe_bytetrack_full_mdmt/", help="test file directory")
    args = parser.parse_args()

    sequences_result = os.listdir(args.sequences_result)
    ...
    sequences_gt = os.listdir("./demo/txt/gt_true/")
    ...
    while(i<len(sequences_result)):
        ...
        if ".txt" in str(sequences_result[i]):
            i = i+1
            continue
        result_path_A = args.sequences_result + sequences_result[i]
        result_path_B = args.sequences_result + sequences_result[i+1]
        gt_path_A = "./demo/txt/gt_true/" + sequences_result[i].split(".")[0] + ".txt"
        gt_path_B = "./demo/txt/gt_true/" + sequences_result[i+1].split(".")[0] + ".txt"
